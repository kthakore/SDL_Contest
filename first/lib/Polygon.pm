...
sub _construct {
	...
	$surf->draw_rect( [0,0,$surf->w, $surf->h], $self->{bgcolor} ) if $self->{bgcolor};
	SDL::GFX::Primitives::filled_polygon_color( $self->{surf}, $poly_points->[0], $poly_points->[1], $self->{verts} + 1, $self->{color} );
	SDL::GFX::Primitives::aapolygon_color( $self->{surf}, $poly_points->[0], $poly_points->[1], $self->{verts} + 1, $self->{color} );
