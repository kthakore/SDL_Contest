...
$app->stash->{ polygons } = []; 
$app->add_show_handler( sub { $app->draw_rect([0,0,$app->w, $app->h], 0 ); } );

foreach( 0..5)
{
my $sides = (rand() * 5 )+ 3;
my $color = int( rand() * 0xffffffff ) + 0x11111111;
my $tri = Polygon->new( verts => $sides, color => $color, bgcolor => 0x00000011, radius => 20, width => 200, height => 200, depth => 32 );
